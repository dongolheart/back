...
//to view particulat order

exports.orderDetails = async (req, res) => {
    let order = await Order.findById(req.params.id).populate('user', 'user_name').populate({
...
exports.userOrder = async(req,res) => {
    let order =await Order.findOne({user:req.params.user}).populate({path:'orderItems',populate:{path:'product',populate:'category'}}).sort({
        'createdAt':'-1'})
        if(!order){
